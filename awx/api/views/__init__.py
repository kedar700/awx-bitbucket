...
class DashboardView(APIView):
    ...
    def get(self, request, format=None):
        ...
        failed_inventory = user_inventory.aggregate(Sum('inventory_sources_with_failures'))[
                               'inventory_sources_with_failures__sum'] or 0
        ...
        data['hosts'] = {'url': reverse('api:host_list', request=request),
                         'failures_url': reverse('api:host_list',
                                                 request=request) + "?last_job_host_summary__failed=True",
        ...
        data['scm_types']['git'] = {'url': reverse('api:project_list', request=request) + "?scm_type=git",
                                    'label': 'Git',
                                    'failures_url': reverse('api:project_list',
                                                            request=request) + "?scm_type=git&last_job_failed=True",
        ...
        data['scm_types']['svn'] = {'url': reverse('api:project_list', request=request) + "?scm_type=svn",
                                    'label': 'Subversion',
                                    'failures_url': reverse('api:project_list',
                                                            request=request) + "?scm_type=svn&last_job_failed=True",
        ...
        data['scm_types']['hg'] = {'url': reverse('api:project_list', request=request) + "?scm_type=hg",
                                   'label': 'Mercurial',
                                   'failures_url': reverse('api:project_list',
                                                           request=request) + "?scm_type=hg&last_job_failed=True",
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if instance.controlled_groups.count():
            raise PermissionDenied(detail=_(
                "Instance Groups acting as a controller for an Isolated Group can not be removed from the API"))
...
class TeamRolesList(SubListAttachDetachAPIView):
    ...
    def get_queryset(self):
        ...
        return models.Role.filter_visible_roles(self.request.user,
                                                team.member_role.children.all().exclude(pk=team.read_role.pk))
    ...
    def post(self, request, *args, **kwargs):
        ...
        if role.content_type == credential_content_type:
            if not role.content_object.organization or role.content_object.organization.id != team.organization.id:
                data = dict(msg=_(
                    "You cannot grant credential access to a team when the Organization field isn't set, or belongs to a different organization"))
                return Response(data, status=status.HTTP_400_BAD_REQUEST)
...
class TeamProjectsList(SubListAPIView):
    ...
    def get_queryset(self):
        ...
        return self.model.accessible_objects(self.request.user, 'read_role').filter(
            pk__in=[t.content_object.pk for t in proj_roles])
...
class ProjectTeamsList(ListAPIView):
    ...
    def get_queryset(self):
        ...
        all_roles = models.Role.objects.filter(Q(descendents__content_type=project_ct) & Q(descendents__object_id=p.pk),
                                               content_type=team_ct)
        return self.model.accessible_objects(self.request.user, 'read_role').filter(
            pk__in=[t.content_object.pk for t in all_roles])
    ...
    def post(self, request, *args, **kwargs):
        ...
        if obj.can_update:
            ...
            if not project_update:
                ...
                data.update(
                    serializers.ProjectUpdateSerializer(project_update,
...
class TeamCredentialsList(SubListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        visible_creds = models.Credential.accessible_objects(self.request.user, 'read_role')
        team_creds = models.Credential.objects.filter(
            Q(use_role__parents=team.member_role) | Q(admin_role__parents=team.member_role))
...
class EnforceParentRelationshipMixin(object):
    ...
    * Tower uses a shallow (2-deep only) url pattern. For example:
    ...
    def create(self, request, *args, **kwargs):
        ...
        if getattr(data, '_mutable', None) is False:
            data._mutable = True
        data[self.enforce_parent_relationship] = getattr(self.get_parent_object(),
                                                         '%s_id' % self.enforce_parent_relationship)
    ...
    def post(self, request, *args, **kwargs):
        ...
        for inventory_source in inventory.inventory_sources.exclude(source=''):
            ...
            if inventory_source.can_update:
                ...
                details.update(serializers.InventoryUpdateDetailSerializer(update,
                                                                           context=self.get_serializer_context()).to_representation(
                    update))
    ...
    def post(self, request, *args, **kwargs):
        ...
        if obj.can_update:
            ...
            if not update:
                ...
                data.update(serializers.InventoryUpdateDetailSerializer(update,
                                                                        context=self.get_serializer_context()).to_representation(
                    update))
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not result:
            ...
            if isinstance(new_job, models.WorkflowJob):
                ...
                data.update(
                    serializers.WorkflowJobSerializer(new_job, context=self.get_serializer_context()).to_representation(
                        new_job))
            else:
                data['job'] = new_job.id
                data['ignored_fields'] = self.sanitize_for_response(serializer._ignored_fields)
                data.update(serializers.JobSerializer(new_job, context=self.get_serializer_context()).to_representation(
                    new_job))
    ...
    def _validate_spec_data(new_spec, old_spec):
        ...
        for idx, survey_item in enumerate(new_spec["spec"]):
            ...
            if not isinstance(survey_item, dict):
                return Response(dict(error=_("Survey question %s is not a json object.") % str(idx)),
                                status=status.HTTP_400_BAD_REQUEST)
            for field_name in ['type', 'question_name', 'variable', 'required']:
                ...
                if not isinstance(val, allow_types):
                    return Response(
                        dict(error=_("'{field_name}' in survey question {idx} expected to be {type_label}.").format(
            ...
            if qtype == 'multiselect' or qtype == 'multiplechoice':
                if 'choices' in survey_item:
                    if isinstance(survey_item['choices'], str):
                        survey_item['choices'] = '\n'.join(
                            choice for choice in survey_item['choices'].splitlines() if choice.strip() != '')
                else:
                    return Response(dict(error=_(
                ...
                if 'default' in survey_item:
                    if isinstance(survey_item['default'], str):
                        survey_item['default'] = '\n'.join(
                            choice for choice in survey_item['default'].splitlines() if choice.strip() != '')
...
class JobTemplateLabelList(DeleteLastUnattachLabelMixin, SubListCreateAttachDetachAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if 'id' not in request.data and 'name' in request.data and 'organization' in request.data:
            existing = models.Label.objects.filter(name=request.data['name'],
...
class JobTemplateCallback(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if len(matching_hosts) != 1:
            ...
            inventory_update_qs = models.InventoryUpdate.objects.filter(pk__in=inventory_update_pks,
            ...
            for inventory_update in models.InventoryUpdate.objects.filter(pk__in=inventory_update_pks,
                                                                          status='successful'):
...
class WorkflowJobTemplateLaunch(RetrieveAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        data.update(serializers.WorkflowJobSerializer(new_job, context=self.get_serializer_context()).to_representation(
            new_job))
        headers = {'Location': new_job.get_absolute_url(request)}
        return Response(data, status=status.HTTP_201_CREATED, headers=headers)
...
class WorkflowJobRelaunch(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if obj.is_sliced_job:
            ...
            elif not obj.inventory or min(obj.inventory.hosts.count(),
                                          jt.job_slice_count) != obj.workflow_nodes.count():
...
class SystemJobTemplateLaunch(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        data.update(
            serializers.SystemJobSerializer(new_job, context=self.get_serializer_context()).to_representation(new_job))
        headers = {'Location': new_job.get_absolute_url(request)}
        return Response(data, status=status.HTTP_201_CREATED, headers=headers)
        ...
        try:
            ...
            if target_format in ('html', 'api', 'json'):
                ...
                if target_format == 'json':
                    ...
                    return Response(
                        {'range': {'start': start, 'end': end, 'absolute_end': absolute_end}, 'content': content})
                return Response(data)
...
class NotificationTemplateTest(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not notification:
            ...
            data.update(serializers.NotificationSerializer(notification,
                                                           context=self.get_serializer_context()).to_representation(
                notification))
...
class WorkflowApprovalApprove(RetrieveAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if obj.status != 'pending':
            return Response({"error": _("This workflow step has already been approved or denied.")},
                            status=status.HTTP_400_BAD_REQUEST)
        obj.approve(request)
        return Response(status=status.HTTP_204_NO_CONTENT)
