...
class BitbucketWebhookReceiver(WebhookReceiverBase):
    ...
    def get_event_status_api(self):
        if self.get_event_type() != 'pr:merged':
            return
        return self.request.data.get('pullRequest', {}).get('links',{}).get('self')[0].get('href')

    def get_signature(self):
        logger.debug("self.request.headers :{}".format(self.request.headers))
        ...
        hash_alg, signature = header_sig.split('=')
        if hash_alg != 'sha256':
            logger.debug("Unsupported signature type, expected: sha256, received: {}".format(hash_alg))
            raise PermissionDenied
        return force_bytes(signature)
